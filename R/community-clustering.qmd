  ...
  html: 
    ...
    code-links:
      - text: kbvernon/community-centers
        icon: github
        href: https://github.com/kbvernon/community-centers
execute: 
  echo: true
  warning: false
  error: false
...
add_graph <- function(x, max_slope = NULL){
  ...
  # Campbell's hiking function, see Campbell et al 2022
  # note: campbell function wants degrees!
  campbell <- function(x) {
    
    # values provided in Appendix Table S3
...
![](../figures/boundary-example.png){width="100%"
fig-alt="An example of the method used to define community boundaries."}
...
cmv <- cmv |> define_boundaries(concavity_ratio = 0.5, dilate = c(130, -100))
nrg <- nrg |> define_boundaries(concavity_ratio = 0.5, dilate = c(130, -100))
cib <- cib |> define_boundaries(concavity_ratio = 0.5, dilate = c(130, -100))
