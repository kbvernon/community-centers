...
landscape, but that is not as bad as it sounds. The idea that we could build a
model that is perfectly isomorphic with the actual or true path taken by people
a thousand years ago is wildly implausible. The idea of the 'true path' is also
problematic as we're not evaluating a single commute, but rather the path people
should take more often than not. So, the real question is what level of
precision is sufficient for evaluating that. And the answer is, of course, it
just depends.
...
urban centers, probably the difference between \~30 m and \~80 m grid cells
...
To calculate travel time between every pair of site points, we use a
bi-directional version of Dijkstra's algorithm as implemented in the R package
`{cppRouting}`. Note that we also average the from- and to- travel times, so the
results are isotropic (meaning direction is irrelevant).
...
Step 1 is already done and stored in the data. Step 2 can technically be done
after 2 and 3, but it makes sense to do it first, since it means those sites can
...
Here the catchment room count, $N$, of focal center $c$ is defined as

$$N_{c} = \sum_{k=1}^{S} R_{k} \cdot I(t_{kc} \leq D)$$

for all sites $S$ (including both farms and centers), with $R_{k}$ being the
room count at site $k$, $t_{kc}$ the travel time from $k$ to $c$, $D$ the
...
$$N_{cd} = \sum_{k=1}^{S} R_{k} \cdot I(t_{kc} \leq D) \cdot I(t_{kd} \leq D)$$
